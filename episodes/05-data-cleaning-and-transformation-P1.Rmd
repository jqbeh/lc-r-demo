...
setwd("your_working_directory")  # navigate to your local working directory
dir_create("data")
...
Don't forget to also load the `tidyverse` package and data frame into your R session.
...
`dplyr` is also part of the `tidyverse.` We're going to learn some of the most 
common **`dplyr`** functions in this episode:
...
print the names of the `books` dataset you can see that some of the variable (column)
names are not particularly helpful or self-explanatory:
...
in the `dplyr` package is the easiest and most straightforward. Here, we want 
to rename the X245.ab variable. Make sure you assign the output to your
...
Use `rename()` to rename the `CALL...ITEM.` column to `callnumber2`. 
Remember to add the period to the end of the `CALL...ITEM.` value
...
in the `books` dataset, the `subCollection` (formerly `BCODE1`) and `format` (formerly `BCODE2`) variables contain single characters.
...
we are overwriting the values in the `books$subCollection` variable.
...
Now do the same for the `format` column. Note that you must put `"5"` and `"4"` into
...
```{r, comment=FALSE, purl=FALSE}
booksOnly <- filter(books, format == "book") # filter books to return only those 
items where the format is books
```

You can also use multiple filter conditions. However, the order matters: first we
...
the `format` is filtered to include only `book`, then only the `title` and 
`tot_chkout` columns are selected, and finally the data is rearranged from 
most to least checkouts.
...
version of the `books` dataset with most of the changes we made above. We will first 
read in the original, then make all the changes with pipes.
...
- Run `vignette("dplyr")` after loading the `dplyr` package to get more 
detailed explanation.
- See the ["Data Transformation" chapter](http://r4ds.had.co.nz/transform.html) in Garrett Grolemund and Hadley Wickham's book *R for Data Science.*
- Watch this Data School video: [Hands-on dplyr tutorial for faster data manipulation in R](https://www.youtube.com/watch?v=jWjqLW-u3hc)
