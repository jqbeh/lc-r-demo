...
as ["a
language for data analysis and graphics."](https://doi.org/10.2307/1390807) You can use R to create, import, and
...
## Discussion: R and librarianship
...
Integrated Development Environment (IDE) for working with R. The RStudio IDE open-source product is free under the
...
3. For "Directory name:" enter **library_carpentry**. This will be your **working 
   directory** for the rest of the day.

4. Finally, click on <kbd>Create project<kbd>. Create a new file where we will type 
   our scripts. Go to <kbd>File > New File > R<kbd> script. Click the save icon on 
...
The **prompt (`>`)** is the blinking cursor in the console that prompts you to take
action. If R is ready to accept a **command**, the R console shows a `>` prompt. If R receives 
a command, R will execute it and, when the results come back, a new `>` prompt will appear 
...
the <kbd>Ctrl</kbd> + <kbd>Enter</kbd> shortcut (for **Windows**) or <kbd>Cmd</kbd> +
<kbd>Return</kbd> (for **Mac**). 

The command on the current line in the script (indicated by the cursor) or all of the 
commands in selected text will be sent to the console and executed when you press
<kbd>Ctrl</kbd> + <kbd>Enter</kbd>. If there is information in the console you do not 
need anymore, you can clear it with <kbd>Ctrl</kbd> + <kbd>L</kbd>. 
...
See [this page from RStudio](https://support.rstudio.com/hc/en-us/articles/201057987-Quick-list-of-useful-R-packages)
for a good list of useful R packages. In addition to CRAN, R users can make
their code and packages available from [GitHub](https://github.com/trending/r) or
...
This is ticked by default, which is usually what you want. Packages can (and do) make 
use of functionality built into other packages, so for the functionality contained in
the package you are installing to work properly, there may be other packages which have 
to be installed with them. The `Install dependencies` option makes sure that this happens.
...
command directly in the R console. Run `help(install.packages)` to learn more about how to do it this way.
...
You can also find the cheat sheets in RStudio by going to the <kbd>Help<kbd> panel then 
...
- Data Carpentry's ["Intro to R and RStudio for Genomics"](https://datacarpentry.github.io/genomics-r-intro/instructor/index.html)


:::::::::::::::::::::::::::::::::::::::: keypoints

- Navigate round RStudio and create an `Rproj` file.
- Use RStudio to write and run R commands.
