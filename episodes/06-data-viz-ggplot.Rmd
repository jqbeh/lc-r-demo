...
If you have not already done so, open your R Project file (`library_carpentry.Rproj`) created in the `Before We Start` lesson. Ensure that you have the following sub-directories in your Rproj: `data\`, `data_output\` and `fig_output\`. Load the data and packages required for this lesson:
You can download the dataset from the web by running:
...
lesson. You can create the reformatted data by running the codes from [here](https://librarycarpentry.github.io/lc-r/03-data-cleaning-and-transformation.html#exporting-data) or by loading the saved CSV file from previous episode.

```{r, purl=FALSE, eval=FALSE}
books2 <- read_csv("data_output/books_reformatted.csv")  # load the data and assign it to books2
```
...
R, so is data visualization with `ggplot2`. `ggplot2` is a
["grammar" for data visualization](https://vita.had.co.nz/papers/layered-grammar.html) also created by Hadley Wickham, as an implementation of Leland Wilkinson's [Grammar of Graphics](https://www.worldcat.org/oclc/934513040).
...
After running this code, a blank canvas is being created. Next, we define a mapping aesthetic with the `mapping` argument (using the aesthetic (`aes()`) function). This defines the variables to be plotted and specifies how to present them in the graph, e.g. as
...
Save the plot into the `fig_output/` folder in your working directory.
